...
// Set bot commands for the menu button (optional; inline menu covers all)
bot.setMyCommands([
...
// Live rates cache + fetcher using CoinPayments "rates" (BTC-base) for BTC/ETH; USDT uses fixed rates
let lastRates = null;
let lastRatesAt = 0;
...
// --- INLINE MENU RENDERERS ---

async function renderMainMenu(chatId) {
  const dateTime = getCurrentDateTime();
  const name = userStates[chatId]?.firstName || ''; // best effort
...
async function renderSupportPrompt(chatId) {
  if (userStates[chatId] && userStates[chatId].awaiting) {
    await sendOrEditMessage(chatId, "⚠️ You’re in the middle of a transaction. Please /start a new one or finish first, then message support.");
    return;
  }
...
// /start (supports referrals) -> shows inline main menu
...
// /find -> same as inline route
bot.onText(/\/find/, async (msg) => {
  await renderFindTransactionPrompt(msg.chat.id);
});

// /referral (kept as-is, but add "Main Menu" button)
...
// /help -> same as inline route
bot.onText(/\/help/, async (msg) => {
  await renderHelp(msg.chat.id);
});

// /support -> same as inline route
bot.onText(/\/support/, async (msg) => {
  await renderSupportPrompt(msg.chat.id);
});

// Callback queries (menu-first routing)
bot.on('callback_query', async (callbackQuery) => {
  ...
  // If coming from support input, let user switch screens
  if (userStates[chatId].awaiting === 'support_message' && data !== 'menu') {
    delete userStates[chatId].awaiting;
  }

  try {
    // High-level menu routes
    if (data === 'menu') {
      await renderMainMenu(chatId);
    } else if (data === 'show_help') {
      await renderHelp(chatId);
    } else if (data === 'show_about') {
      await renderAbout(chatId);
    } else if (data === 'find_transaction') {
      await renderFindTransactionPrompt(chatId);
    } else if (data === 'support_open') {
      await renderSupportPrompt(chatId);
    ...
    // Start or Refresh rates
      ...
      await sendOrEditMessage(chatId, "No worries! Whenever you’re ready, just send /start to begin again.", {
        reply_markup: { inline_keyboard: [[{ text: "🏠 Main Menu", callback_data: 'menu' }]] }
      });
    ...
    // Network picked (TRC20/ERC20) → ask for amount
      ...
      switch (method) {
        ...
        case 'paypal':
          ...
          break;
        case 'bank':
          await sendOrEditMessage(chatId, "✅ *Bank Transfer Selected*\n\nPlease choose your bank’s region:", {
            reply_markup: {
              inline_keyboard: [
          ...
          break;
        case 'skrill':
          await sendOrEditMessage(chatId, "✅ *Skrill/Neteller Selected*\n\nWhich one are you using?", {
            reply_markup: {
              inline_keyboard: [
